...
def test_self_attention():
    mha = MultiHeadAttention(d, h, 0.0).to(device)
    x = torch.rand(b, s, d).to(device)
    q, kT, v = mha.q_kT_v(x)
    ...
    attn = mha.self_attention(q, kT, v, attention_mask=attention_mask.to(device))
    attn_ref_multihead = F.scaled_dot_product_attention(
        q, k, v, attn_mask=attention_mask_with_causal.to(device)
    )
    attn_ref = rearrange(attn_ref_multihead, "b h s hd -> b s (h hd)")
...
def test_lm_forward_on_cpu():
    ...
    logits = lm(iids)
    logits_masked = lm(iids, attention_mask=torch.ones_like(iids, dtype=torch.float).to(device))
