...
def random_batch_sampler(
    ...
    while True:
        offsets = torch.randint(len(tokens) - seq_len + 1, size = (batch_size, ))
        x = torch.stack([tokens[i: i + seq_len] for i in offsets], dim = 0).to(device)
        yield x
...
def sequential_batch_sampler(
    ...
    Note: If the last batch is incomplete, which could happen when the number
        ...
        the last batch.
    """
    rem = len(tokens) % (batch_size * seq_len)
    if rem != 0:
        tokens = tokens[:-rem]
    batched_tokens = tokens.reshape(-1, batch_size, seq_len).to(device)
...
def train(
    ...
    autocast: Union[torch.autocast, nullcontext] = nullcontext(),
...
def main():
    ...
    wandb.init(project="llms-hw3", config=OmegaConf.to_container(config))
    ...
    tokens = np.load("HW3/llms-class-hw-3-main/dataset/tokens.npz")
